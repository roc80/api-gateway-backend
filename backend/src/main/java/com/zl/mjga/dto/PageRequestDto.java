        ...
        public static Direction fromString(String value) {
            try {
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "Invalid value '%s' for orders given; Has to be either 'desc' or"
    ...
    private Map<String, Direction> convertSortBy(String sortBy) {
        ...
        for (String fieldSpaceDirection : sortBy.split(",")) {
            ...
            if (fieldDirectionArray.length != 2) {
                throw new IllegalArgumentException(
                        String.format(
                                "Invalid sortBy field format %s. The expect format is [col1"
                                        + " asc,col2 desc]",
                                sortBy));
            }
            ...
            if (!verifySortField(field)) {
                throw new IllegalArgumentException(
                        String.format(
                                "Invalid Sort field %s. Sort field must match %s", sortBy, REGEX));
            }
