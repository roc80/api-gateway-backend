...
import org.jooq.generated.api_gateway.tables.daos.RoleDao;
...
public class RoleRepository extends RoleDao {
    ...
    public Result<Record> pageFetchRolesBy(
            PageRequestDto pageRequestDto, RoleQueryDto roleQueryDto) {
        return ctx().select(asterisk(), DSL.count(ROLE.ID).over().as("total_role"))
                .from(ROLE)
                .where(
                        CollectionUtils.isEmpty(roleQueryDto.getRoleIdList())
                                ? noCondition()
    ...
    public Result<Record> getRoleAggBy(Long roleId) {
        return ctx().select(ROLE.asterisk(), ROLE.permission().asterisk())
                .from(ROLE)
